  ...
  return (
    <div className="flex flex-col h-full bg-card">
          ...
          >
            ...
            {gallery.map(item => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${selectedCategory === item.type
                  ? "bg-primary text-primary-foreground"
                  : "bg-muted text-muted-foreground hover:bg-muted/80"
                  }`}
              >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {filtered.map((item) => {
          if (selectedCategory !== "all" && selectedCategory !== item.type) return null;
                    ...
                    <div className="aspect-[2/1] overflow-hidden bg-muted relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            onAddComponent(item.type, variant.id)
                          }}
